...
def generate_graph(edge_index, selected_impedges_idx, Generator, features):
    sub_edge_index = edge_index[:, selected_impedges_idx]
    if sub_edge_index.shape[1]< edge_index.shape[1]:
        sparsity_edges = sub_edge_index.shape[1] / edge_index.shape[1]
        sub_edge_mask = torch.ones(sub_edge_index.shape[1], dtype=torch.float32).reshape(-1,1)
        origin_edge_mask = torch.ones(edge_index.shape[1], dtype=torch.float32).reshape(-1,1)
        mu, log_var, z = Generator.encode(x=features, in_edge_index=sub_edge_index, 
            in_edge_attr=sub_edge_mask, reparameterize=1
            )
        _, _, cond_z = Generator.encode(x=features, in_edge_index=edge_index, 
            in_edge_attr=origin_edge_mask, reparameterize=1
            )
        z = torch.cat([z, cond_z], dim=1)
        fake_edge_index, fake_edge_prob, fake_edge_attr, _ = Generator.fill(z=z, preserved_edge_index=sub_edge_index, 
            preserved_edge_ratio=np.array([sparsity_edges]),
            batch=torch.zeros(features.shape[0], dtype=torch.int64, device=features.device), pos=None, neg_edge_index=None, threshold=False
            )
    else:
        fake_edge_index = sub_edge_index
...
def attack_graph(edge_index, edge_mask, top_k, perturb_edge_num):
    ...
    while 1==1:
        ...
        if len(sub_nodes)==0 or (start_node in sub_nodes and end_node not in sub_nodes) or (start_node not in sub_nodes and end_node in sub_nodes):
            attacked_edge_index = torch.cat((attacked_edge_index, torch.tensor([[start_node], [end_node]])), axis=1)
...
class MaskoutMetric:
    ...
    #def metric_del_edges_GC(self, topk_arr, sub_feature, mask, sub_edge_index, origin_pred, masked_pred, label, Generator):
    ...
    def metric_del_edges_GC_distribution(self, topk_arr, sub_feature, mask, sub_edge_index, origin_pred, masked_pred, label, Generator):
        ...
        for top_k in topk_arr:
            ...
            data = Data(x=x, edge_index=sub_edge_index, batch = torch.zeros(x.shape[0], dtype=torch.int64, device=x.device))
            self.model.eval()
            _, masknotimp_preds, embed = self.model(data)
            masknotimp_pred = masknotimp_preds.squeeze()
            self.__clear_masks__()
            ...
            data = Data(x=x, edge_index=sub_edge_index, batch = torch.zeros(x.shape[0], dtype=torch.int64, device=x.device))
            self.model.eval()
            _, delimp_preds, embed = self.model(data)
            delimp_pred = delimp_preds.squeeze()
            self.__clear_masks__()
